	...
	for y = 0; int(y) < len(term.cells); y++ {
		for x = 0; int(x) < len(term.cells[y]); x++ {
			if term.cells[y][x].char != 0 {
				...
				err = term.renderer.PrintRuneColor(term.cells[y][x].char, x, y, fg, bg, term.cells[y][x].sgr.bitwise)
			} else {
				fg, bg := sgrOpts(SGR_DEFAULT)
				err = term.renderer.PrintRuneColor(' ', x, y, fg, bg, 0)
			}
			if err != nil {
...
func (term *Term) _blinkCursor() {
	...
	err := term.renderer.PrintRuneColor(r, term.curPos.X, term.curPos.Y, fg, bg, style)
	if err != nil {
